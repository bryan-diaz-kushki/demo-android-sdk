...
class RequestTransactionTokenAsyncTask : AbstractRequestTokenAsyncTask {
    ...
    override fun requestToken(card: Card): Transaction {
        val card = Card("'${$('#credit-card-name').val()}'", "'${$('#credit-card-number').val()}'", "***", "'${$('#card-date').val().split('/')[0]}'", "'${$('#card-date').val().split('/')[1]}'") //add card properties

        val totalAmount = ${totalG}
        val publicMerchantId = "'${merchantKey}'"
        val kushki = Kushki(publicMerchantId,
                "'${currency}'",
                KushkiEnvironment.TESTING,
                false) //optional parameter regional

        return runBlocking { kushki.requestToken(card,totalAmount, _context, _activity, true) }
    }
